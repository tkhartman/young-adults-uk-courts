##' ---
##' title: "MoJ Young Adults Data Analysis Project"
...
pacman::p_load(broom, data.table, ggplot2, 
               googleVis, magicfor, questionr)

## Load MoJ data
# df <- fread("data-behind-interactive-data-tools/Court outcomes by PFA 2017.csv")
load("Court_outcomes_by_PFA_2017.RData")
head(df)  # Check the data loaded properly
colnames(df)  # Check the variable names
...
## Expand the data based on the 'Count variable
## This makes a massive file with 18m + obervations
# df.full <- df[rep(1:nrow(df), df$Count), ]

## Subset data by indviduals and non-summary offenses
...
## Confirm subsetting worked properly
...
dframe <- vector(mode = "list", 11)  # Store subsetted dataframes in list 
year <- seq(from = 2007, to = 2017, by = 1)
...
age.lab.1 <- rep(c("Youth", "Young Adult", "Adult"), times = 11)
...
age.lab.2 <- rep(c("Youth", "Young Adult", "Adult"), times = 22)
court.lab <- rep(c("Crown Court", "Magistratres Court"), times = 11, each = 3) 
...
court.lab.b <- rep(c("Crown Court", "Magistratres Court"), times = 33) 
age.lab.2.b <- rep(c("Youth", "Young Adult", "Adult"), times = 11, each = 2)
...
convicted.lab <- rep(c("Convicted", "Not Convicted"), times = 11, each = 3)
...
#### WITHIN AGE GROUPS
convicted.lab.b <- rep(c("Convicted", "Not Convicted"), times = 33)
...
sentenced.lab <- rep(c("Sentenced", "Not Sentenced"), times = 11, each = 3)
...
#### WITHIN AGE GROUPS
sentenced.lab.b <- rep(c("Sentenced", "Not Sentenced"), times = 33)
...
age.lab.3 <- rep(c("Youth", "Young Adult", "Adult"), times = 44)
...
outcome.lab.b <- rep(c("Discharged", "Community Sentence", "Immediate Custody", "Other"), 
                   times = 33)
age.lab.3.b <- rep(c("Youth", "Young Adult", "Adult"), times = 11, each = 4)
...
age.lab.4 <- rep(c("Youth", "Young Adult", "Adult"), times = 110)
...
offence.lab.b <- rep(c("Violence", "Sexual", "Robbery", "Theft", "Damage", 
                       "Drug", "Weapons", "Public Order", "Miscellaneous", 
                       "Fraud"), times = 33)
age.lab.4.b <- rep(c("Youth", "Young Adult", "Adult"), times = 11, each = 10)
...
######################################################################
pop <- read.csv("nomis_2018_11_27_154545_v3_WITH_TOTALS.csv")
out.pop <- out1
out.pop <- out.pop[, -4]
...
combined$Type <- factor(combined$Type, 
                        levels = c("System", "Crown Court", "Magistratres Court",
                                   "Convicted", "Sentenced", "Immediate Custody",
...
#######  REGRESSION MODELS ###########################################
######################################################################
model <- vector(mode = "list", 11)  # Store subsetted dataframes in list 
...
######################################################################
#######  REGRESSION MODELS - YOUNG ADULTS ONLY########################
######################################################################
dframe.ya <- vector(mode = "list", 11)  # Store subsetted dataframes in list 
year.ya <- seq(from = 2007, to = 2017, by = 1)
for (j in 1:length(year.ya)) {
    dframe.ya[[j]] <- subset(df.sub, `Year of Appearance` == year.ya[j] & 
                                age == "2 - Young Adult")
}

model.ya <- vector(mode = "list", 11)  # Store subsetted dataframes in list 
output.ya <- data.frame("term" = NA, "estimate" = NA, "std.error" = NA, 
                     "statistic" = NA, "p.value" = NA, "OR" = NA, "Year" = NA)

for (j in 1:length(year.ya)) {
    df.ya <- dframe.ya[[j]]
    indiv.data.ya <- df.ya[ rep(1:nrow(df.ya), df.ya[["Count"]]), 
                      c("Police Force Area", "Year of Appearance", "Quarter",
                        "Type of Defendant", "Sex", "Age Group",
                        "Age Range", "Ethnicity", "Court Type",
                        "Offence Type", "Offence Group", "offence",
                        ...
                        "white", "black", "asian", 
                        "mixed", "chinese", "ethn.unknown"  ) ]
    model.ya[[j]] <- glm(convicted ~ female + sex.unknown + 
                          black + asian + mixed + chinese + ethn.unknown + 
                          crown.court, 
                      family = binomial(link = "logit"),
                      data = indiv.data.ya )
    ...
    cat("\n", "Year", year[[j]], "\n")
    print(model.ya[[j]])
    model.ya[[j]] <- tidy(model.ya[[j]])
    model.ya[[j]][["OR"]] <- exp(model.ya[[j]][["estimate"]])
    model.ya[[j]][["Year"]] <- year.ya[[j]]
    output.ya <- rbind(output.ya, model.ya[[j]])
}

output.ya <- output.ya[-1, ]
write.csv(output.ya, "moj_regression_results_young_adults_only.csv")

## Plot by gender
output.ya <- as.data.frame(output.ya)
output.ya.female <- subset(output.ya, term == "female")

p8 <- ggplot(data = output.ya.female, 
                 aes(x = factor(Year, levels = seq(from = 2007, to = 2017, by = 1)), 
                     y = OR, group = 1)) + 
    geom_point() +
    geom_line() + 
    ...
    ylab("Odds Ratio") +
    xlab("Year") +
    ggtitle("Likelihood of Conviction for Female Young Adult Offenders") +
    scale_x_discrete(breaks = c("2007","2008","2009", "2010", "2011", "2012",
                                "2013", "2014", "2015", "2016", "2017"),
                     labels = c("07","08","09", "10", "11", "12",
                                "13", "14", "15", "16", "17"))
...
p8
ggsave("moj_results_ODDS_RATIO_FEMALES_year.png", dpi = 320, 
       scale = 1.5)


## Plot by BLACK
output.ya.black <- subset(output.ya, term == "black")

p8b <- ggplot(data = output.ya.black, 
             aes(x = factor(Year, levels = seq(from = 2007, to = 2017, by = 1)), 
                 y = OR, group = 1)) + 
    geom_point() +
    geom_line() + 
    ...
    ylab("Odds Ratio") +
    xlab("Year") +
    ggtitle("Likelihood of Conviction for Black Young Adult Offenders") +
    scale_x_discrete(breaks = c("2007","2008","2009", "2010", "2011", "2012",
                                "2013", "2014", "2015", "2016", "2017"),
                     labels = c("07","08","09", "10", "11", "12",
                                "13", "14", "15", "16", "17"))
...
p8b
ggsave("moj_results_ODDS_RATIO_BLACK_year.png", dpi = 320, 
       scale = 1.5)

## Plot by ASIAN
output.ya.asian <- subset(output.ya, term == "asian")

p8c <- ggplot(data = output.ya.asian, 
              aes(x = factor(Year, levels = seq(from = 2007, to = 2017, by = 1)), 
                  y = OR, group = 1)) + 
    geom_point() +
    geom_line() + 
    ...
    ylab("Odds Ratio") +
    xlab("Year") +
    ggtitle("Likelihood of Conviction for Asian Young Adult Offenders") +
    scale_x_discrete(breaks = c("2007","2008","2009", "2010", "2011", "2012",
                                "2013", "2014", "2015", "2016", "2017"),
                     labels = c("07","08","09", "10", "11", "12",
                                "13", "14", "15", "16", "17"))
...
p8c
ggsave("moj_results_ODDS_RATIO_ASIAN_year.png", dpi = 320, 
       scale = 1.5)

## Plot by CHINESE
output.ya.chinese <- subset(output.ya, term == "chinese")

p8d <- ggplot(data = output.ya.chinese, 
              aes(x = factor(Year, levels = seq(from = 2007, to = 2017, by = 1)), 
                  y = OR, group = 1)) + 
    geom_point() +
    geom_line() + 
    ...
    ylab("Odds Ratio") +
    xlab("Year") +
    ggtitle("Likelihood of Conviction for Chinese Young Adult Offenders") +
    scale_x_discrete(breaks = c("2007","2008","2009", "2010", "2011", "2012",
                                "2013", "2014", "2015", "2016", "2017"),
                     labels = c("07","08","09", "10", "11", "12",
                                "13", "14", "15", "16", "17"))
...
p8d
ggsave("moj_results_ODDS_RATIO_CHINESE_year.png", dpi = 320, 
       scale = 1.5)


## Plot by MIXED RACE
output.ya.mixed <- subset(output.ya, term == "mixed")

p8e <- ggplot(data = output.ya.mixed, 
              aes(x = factor(Year, levels = seq(from = 2007, to = 2017, by = 1)), 
                  y = OR, group = 1)) + 
    geom_point() +
    geom_line() + 
    ...
    ylab("Odds Ratio") +
    xlab("Year") +
    ggtitle("Likelihood of Conviction for Mixed Ethnicity Young Adult Offenders") +
    scale_x_discrete(breaks = c("2007","2008","2009", "2010", "2011", "2012",
                                "2013", "2014", "2015", "2016", "2017"),
                     labels = c("07","08","09", "10", "11", "12",
                                "13", "14", "15", "16", "17"))
...
p8e
ggsave("moj_results_ODDS_RATIO_MIXED_year.png", dpi = 320, 
       scale = 1.5)
...
## Loop to subset data by year 
dframe.m <- vector(mode = "list", 11)  # Store subsetted dataframes in list 
for (j in 1:length(year)) {
...
pop.m <- read.csv("nomis_2019_04_05_181504_Age_by_MALE_Populations.csv")
out.pop.m <- out1.m
...
pop.f <- read.csv("nomis_2019_04_05_181504_Age_by_FEMALE_Populations.csv")
out.pop.f <- out1.f
...
pop.w <- read.csv("nomis_2019_05_01_194620_WHIITE.csv")
...
p1.pop.w <- ggplot(data = out.pop.w, 
                   aes(x = factor(Year, levels = seq(from = 2009, to = 2017, by = 1)), 
                       y = Rate, group = Age, color = Age)) + 
    geom_point() +
    geom_line() + 
    xlab("Year") +
...
################################################################################
## No ethnicity data for 2007 and 2008
year.nw <- seq(from = 2009, to = 2017, by = 1)
## No population estimates for Youth (under 16 not available in the APS)
year.lab.nw <- rep(year.nw, times = 1, each = 2)
age.lab.nw <- rep(c("Young Adult", "Adult"), times = 9)
...
## Loop to subset data by year 
dframe.nw <- vector(mode = "list", 9)  # Store subsetted dataframes in list 
...
pop.nw <- read.csv("nomis_2019_05_01_194620_NONWHIITE.csv")
## Remove 2007 and 2008
pop.nw <- pop.nw[5:22, ]
out.pop.nw <- out1.nw
...
dframe.w2 <- vector(mode = "list", 9)  # Store subsetted dataframes in list 
for (j in 1:length(year.w)) {
...
out4.w <- out4.w[, -1]
sentenced.lab.w <- rep(c("Immediate Custody"), times = 18, each = 1)
year.lab.2.w <- rep(year.w, times = 1, each = 2)
age.lab.2.w <- rep(c("Young Adult", "Adult"), times = 9)
...
##############################################################################################
## Using data from previous white/nonwhite subsets (but only for immediate custody)
dframe.nw2 <- vector(mode = "list", 9)  # Store subsetted dataframes in list 
for (j in 1:length(year.nw)) {
...
out4.nw <- out4.nw[, -1]
sentenced.lab.nw <- rep(c("Immediate Custody"), times = 18, each = 1)
year.lab.2.nw <- rep(year.nw, times = 1, each = 2)
age.lab.2.nw <- rep(c("Young Adult", "Adult"), times = 9)
